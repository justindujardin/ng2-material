...
export class ExampleComponent implements AfterViewInit {
  ...
  @Input() portalSlot: ComponentPortal<any> = null;
  ...
  ngOnDestroy() {
    if (this.portalSlot) {
      this.portalSlot.detach();
      this.portalSlot = null;
    }
  }
  ...
  applyModel(model: IExampleData) {
    ...
    // Import demo module before anything else
    System.import(model.module).then((module: any) => {
      if (!module.default) {
        console.warn(`The demo module (${model.module}) does not have a default exported component`);
        return;
      }
      this.orderedFiles = [];
      this._loaded = false;
      ...
      this.portalSlot = new ComponentPortal(module.default);
