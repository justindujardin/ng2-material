...
export class MdMenu {
  ...
  initMenu() {
    ...
    this._triggerElement && this._triggerElement.setAttribute('aria-haspopup', 'true');

    this._menuContent = this._menuElement.querySelector('md-menu-content');

    for (var i = 0; i < (<HTMLElement>this._menuContent).children.length; i++) {
    ...
    this._menuContainers = this._renderer.createElement(body,'div');
    ...
    DOM.appendChild(body, this._menuContainers);
    ...
    this._renderer.listen(body, 'click', (event) => {
      let rect = this._menuContainers.getBoundingClientRect();
      let isInMenu = (rect.top <= event.clientY && event.clientY <= rect.top + rect.height && rect.left <= event.clientX && event.clientX <= rect.left + rect.width);

      if (this._isOpen && !isInMenu) {
        this.closeMenu();
  ...
  closeMenu() {
    DOM.setStyle(this._menuContainers, 'display', 'none');
    this._isOpen = false;
  }
